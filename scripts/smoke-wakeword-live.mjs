...
const expectedPhrase = process.env.WAKE_WORD_PHRASE ?? "你好小慧";
...
function assertLiveReady(status) {
  const failures = [];
  if (status.provider !== "sherpa-onnx") failures.push(`provider=${status.provider}`);
  ...
  if (!allowBuiltinModel && !status.sherpa_onnx_encoder) failures.push("sherpa_onnx_encoder=missing");
  if (!allowBuiltinModel && !status.sherpa_onnx_keywords) failures.push("sherpa_onnx_keywords=missing");
